...
resource "aws_cloudwatch_metric_alarm" "cf_4xx_alarm" {
    ...
    alarm_name          = "4xx-CloudWatch-Alarm" // Name des CloudWatch Alarms, hier mit Bezug zu den 4xx-Fehlern der CloudFront Distribution
    ...
    threshold           = 5                      // Schwellenwert für die Fehlerquote, hier 5%, um eine Benachrichtigung auszulösen, wenn die Fehlerquote ungewöhnlich hoch ist
...
resource "aws_cloudwatch_metric_alarm" "cf_5xx_alarm" {
    ...
    alarm_name          = "5xx-CloudWatch-Alarm"
...
resource "aws_cloudwatch_metric_alarm" "cf_requests_alarm" {
    provider = aws.us_east_1
    alarm_name          = "Requests-CloudWatch-Alarm"
    comparison_operator = "GreaterThanThreshold"
    evaluation_periods  = 1
    threshold           = 1000
